...
class GaleShapley:
    ...
    def find_matches(self, proposers_preferences : dict[int, list[int]], receivers_preferences : dict[int, list[int]]) -> list[tuple[int, int]]:
        ...
        while free_proposers:
            ...
            if receiver not in matches.values():
                ...
                free_proposers.remove(proposer)
                continue 
            cur_proposer = next(prop for prop, rec in matches.items() if rec == receiver)
            if receivers_preferences[receiver].index(proposer) > receivers_preferences[receiver].index(cur_proposer):
